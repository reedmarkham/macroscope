...
def write_metadata(volume: np.ndarray, tif_path: str, npy_path: str):
    metadata = {
        "source": "epfl",
...
def ingest_epfl_tif():
    tif_path = os.path.join(OUTPUT_DIR, "volumedata.tif")
    npy_path = os.path.join(OUTPUT_DIR, "volumedata.npy")

    download_tif(URL, tif_path)
    volume = load_volume(tif_path)
    save_volume(volume, npy_path)
    write_metadata(volume, tif_path, npy_path)
