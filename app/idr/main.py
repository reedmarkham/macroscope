...
def download_image(session, image_id, output_dir=OUTPUT_DIR):
    image_url = f"https://idr.openmicroscopy.org/webclient/render_image_download/{image_id}/?format=ome.tiff"
    response = session.get(image_url, stream=True)
    image_path = os.path.join(output_dir, f"{image_id}.ome.tiff")
...
def write_metadata(metadata, image_id, shape, volume_path):
    metadata_out = {
        "source": "idr",
...
def ingest_image(image_id):
    ...
    image_path = download_image(session, image_id)
    image_data = load_image(image_path)

    volume_path = os.path.join(OUTPUT_DIR, f"{image_id}.npy")
    np.save(volume_path, image_data)
    print(f"Saved full image volume to {volume_path}")

    write_metadata(metadata, image_id, list(image_data.shape), volume_path)
